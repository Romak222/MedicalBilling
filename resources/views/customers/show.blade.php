<x-layouts.app :title="config('app.name').' Customer Detail'">
    <x-app-shell :page-title="$customer->name" section-label="Customer Detail">
        <x-slot:actions>
            <a href="{{ route('customers.index') }}" class="btn-secondary">Back to Customers</a>
            @if (auth()->user()?->hasPermission('customers.manage'))
                <a href="{{ route('customers.edit', $customer) }}" class="btn-primary">Edit Customer</a>
            @endif
        </x-slot>

        <div class="space-y-5">
            @if (session('status'))
                <div class="surface-panel border-medical-100 bg-medical-50 px-4 py-3 text-sm font-semibold text-medical-800">
                    {{ session('status') }}
                </div>
            @endif

            <section class="surface-panel p-5">
                <div class="flex flex-col gap-3 border-b border-slate-200 pb-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        <p class="section-kicker">Read Only</p>
                        <h2 class="mt-1 text-xl font-semibold text-ink-950">{{ $customer->name }}</h2>
                        <p class="mt-2 text-sm text-slate-600">{{ $customer->code ?: 'No customer code' }}</p>
                    </div>
                    <span class="badge {{ $customer->is_active ? 'bg-medical-50 text-medical-700' : 'bg-slate-200 text-slate-600' }}">
                        {{ $customer->is_active ? 'Active' : 'Deleted' }}
                    </span>
                </div>

                <dl class="mt-5 grid gap-4 lg:grid-cols-3">
                    <div class="rounded-lg border border-slate-200 bg-slate-50/70 p-4">
                        <dt class="text-xs font-semibold uppercase text-slate-500">Phone</dt>
                        <dd class="mt-2 text-sm font-semibold text-ink-900">{{ $customer->phone ?: 'Not set' }}</dd>
                    </div>
                    <div class="rounded-lg border border-slate-200 bg-slate-50/70 p-4">
                        <dt class="text-xs font-semibold uppercase text-slate-500">Email</dt>
                        <dd class="mt-2 text-sm font-semibold text-ink-900">{{ $customer->email ?: 'Not set' }}</dd>
                    </div>
                    <div class="rounded-lg border border-slate-200 bg-slate-50/70 p-4">
                        <dt class="text-xs font-semibold uppercase text-slate-500">GSTIN</dt>
                        <dd class="mt-2 text-sm font-semibold text-ink-900">{{ $customer->gstin ?: 'Not set' }}</dd>
                    </div>
                    <div class="rounded-lg border border-slate-200 bg-slate-50/70 p-4">
                        <dt class="text-xs font-semibold uppercase text-slate-500">Opening Balance</dt>
                        <dd class="mt-2 text-sm font-semibold text-ink-900">{{ number_format((float) $customer->opening_balance, 2) }}</dd>
                    </div>
                    <div class="rounded-lg border border-slate-200 bg-slate-50/70 p-4">
                        <dt class="text-xs font-semibold uppercase text-slate-500">Credit Limit</dt>
                        <dd class="mt-2 text-sm font-semibold text-ink-900">{{ $customer->credit_limit === null ? 'Not set' : number_format((float) $customer->credit_limit, 2) }}</dd>
                    </div>
                    <div class="rounded-lg border border-slate-200 bg-slate-50/70 p-4">
                        <dt class="text-xs font-semibold uppercase text-slate-500">Outstanding</dt>
                        <dd class="mt-2 text-sm font-semibold text-ink-900">{{ number_format((float) $customer->outstanding_balance, 2) }}</dd>
                    </div>
                    <div class="rounded-lg border border-slate-200 bg-slate-50/70 p-4">
                        <dt class="text-xs font-semibold uppercase text-slate-500">Loyalty Points</dt>
                        <dd class="mt-2 text-sm font-semibold text-ink-900">{{ $customer->loyalty_points }}</dd>
                    </div>
                    <div class="rounded-lg border border-slate-200 bg-slate-50/70 p-4">
                        <dt class="text-xs font-semibold uppercase text-slate-500">Consents</dt>
                        <dd class="mt-2 text-sm font-semibold text-ink-900">
                            Reminder {{ $customer->reminder_consent ? 'Yes' : 'No' }},
                            WhatsApp {{ $customer->whatsapp_consent ? 'Yes' : 'No' }},
                            SMS {{ $customer->sms_consent ? 'Yes' : 'No' }}
                        </dd>
                    </div>
                    <div class="rounded-lg border border-slate-200 bg-slate-50/70 p-4">
                        <dt class="text-xs font-semibold uppercase text-slate-500">Linked Patients</dt>
                        <dd class="mt-2 text-sm font-semibold text-ink-900">{{ $customer->patients->count() }}</dd>
                    </div>
                    <div class="rounded-lg border border-slate-200 bg-slate-50/70 p-4 lg:col-span-2">
                        <dt class="text-xs font-semibold uppercase text-slate-500">Address</dt>
                        <dd class="mt-2 text-sm font-semibold text-ink-900">
                            {{ $customer->address_line_1 ?: 'Not set' }}
                            @if ($customer->address_line_2)
                                , {{ $customer->address_line_2 }}
                            @endif
                            @if ($customer->city || $customer->state || $customer->postal_code)
                                <span class="block pt-1 text-slate-700">{{ $customer->city }}{{ $customer->state ? ', '.$customer->state : '' }} {{ $customer->postal_code }}</span>
                            @endif
                        </dd>
                    </div>
                    <div class="rounded-lg border border-slate-200 bg-slate-50/70 p-4">
                        <dt class="text-xs font-semibold uppercase text-slate-500">Billing History</dt>
                        <dd class="mt-2 text-sm font-semibold text-ink-900">{{ $customer->salesInvoices->count() }} bills</dd>
                    </div>
                    <div class="rounded-lg border border-slate-200 bg-slate-50/70 p-4 lg:col-span-3">
                        <dt class="text-xs font-semibold uppercase text-slate-500">Notes</dt>
                        <dd class="mt-2 text-sm font-semibold text-ink-900">{{ $customer->notes ?: 'Not set' }}</dd>
                    </div>
                </dl>
            </section>

            <section class="surface-panel overflow-hidden">
                <div class="border-b border-slate-200 bg-white p-5">
                    <p class="section-kicker">Patients</p>
                    <h2 class="mt-1 text-lg font-semibold text-ink-950">Linked Patient Records</h2>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-left text-sm">
                        <thead class="table-header">
                            <tr>
                                <th class="px-4 py-3">Patient</th>
                                <th class="px-4 py-3">Contact</th>
                                <th class="px-4 py-3">Doctor</th>
                                <th class="px-4 py-3">Status</th>
                                <th class="px-4 py-3 text-right">Actions</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 bg-white">
                            @forelse ($customer->patients->sortBy('full_name') as $patient)
                                <tr>
                                    <td class="px-4 py-3 align-top">
                                        <p class="font-semibold text-ink-950">{{ $patient->full_name }}</p>
                                        <p class="mt-1 text-xs text-slate-500">{{ $patient->patient_code ?: 'No patient code' }}</p>
                                    </td>
                                    <td class="px-4 py-3 align-top text-slate-700">
                                        <p>{{ $patient->phone ?: 'No phone' }}</p>
                                        <p class="mt-1 text-xs text-slate-500">{{ $patient->email ?: 'No email' }}</p>
                                    </td>
                                    <td class="px-4 py-3 align-top text-slate-700">{{ $patient->doctor?->name ?: ($patient->primary_doctor_name ?: 'Not set') }}</td>
                                    <td class="px-4 py-3 align-top">
                                        <span class="badge {{ $patient->is_active ? 'bg-medical-50 text-medical-700' : 'bg-slate-200 text-slate-600' }}">
                                            {{ $patient->is_active ? 'Active' : 'Deleted' }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-3 text-right align-top">
                                        <a href="{{ route('patients.show', $patient) }}" class="rounded-md border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-ink-700 shadow-sm transition hover:border-slate-400 hover:bg-slate-50">
                                            View
                                        </a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="px-4 py-10 text-center text-sm font-medium text-slate-500">No patients linked to this customer.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
            </section>

            <section class="surface-panel overflow-hidden">
                <div class="border-b border-slate-200 bg-white p-5">
                    <p class="section-kicker">Billing</p>
                    <h2 class="mt-1 text-lg font-semibold text-ink-950">Recent Bills</h2>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-left text-sm">
                        <thead class="table-header">
                            <tr>
                                <th class="px-4 py-3">Bill</th>
                                <th class="px-4 py-3">Date</th>
                                <th class="px-4 py-3">Patient</th>
                                <th class="px-4 py-3">Total</th>
                                <th class="px-4 py-3">Status</th>
                                <th class="px-4 py-3 text-right">Actions</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 bg-white">
                            @forelse ($customer->salesInvoices->sortByDesc('invoice_date')->take(10) as $salesInvoice)
                                <tr>
                                    <td class="px-4 py-3 align-top font-semibold text-ink-950">{{ $salesInvoice->invoice_number }}</td>
                                    <td class="px-4 py-3 align-top text-slate-700">{{ $salesInvoice->invoice_date?->format('d M Y') }}</td>
                                    <td class="px-4 py-3 align-top text-slate-700">{{ $salesInvoice->patient_name ?: 'Not set' }}</td>
                                    <td class="px-4 py-3 align-top text-slate-700">{{ number_format((float) $salesInvoice->total_amount, 2) }}</td>
                                    <td class="px-4 py-3 align-top">
                                        <span class="badge {{ $salesInvoice->status === 'finalized' ? 'bg-medical-50 text-medical-700' : 'bg-slate-200 text-slate-600' }}">
                                            {{ ucfirst($salesInvoice->status) }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-3 text-right align-top">
                                        <a href="{{ route('sales-invoices.show', $salesInvoice) }}" class="rounded-md border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-ink-700 shadow-sm transition hover:border-slate-400 hover:bg-slate-50">
                                            View
                                        </a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="6" class="px-4 py-10 text-center text-sm font-medium text-slate-500">No bills linked to this customer yet.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
            </section>
        </div>
    </x-app-shell>
</x-layouts.app>
