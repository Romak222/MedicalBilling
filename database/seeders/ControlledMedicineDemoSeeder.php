...
class ControlledMedicineDemoSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $prescription = Prescription::query()->where('prescription_number', 'RX-DEMO-001')->with('items')->first();

        if (! $actor || ! $customer || ! $patient || ! $doctor || ! $prescription) {
            return;
        }

        $taxRate = TaxRate::query()->firstOrCreate(
            ['name' => 'Controlled Demo Tax'],
            [
                'rate_percent' => '5.00',
                'is_active' => true,
                'created_by' => $actor->id,
        ...
        if (! SalesInvoice::query()->where('invoice_number', 'SI-CM-DEMO-001')->exists()) {
            app(SalesBillingManager::class)->createFinalizedInvoice([
                'sale' => [
                    'invoice_number' => 'SI-CM-DEMO-001',
